...
from MRAG.envs.AttackerDefender import AttackerDefender1vs1
...
avoid_set = agents_1v1.capture_set(grids, 0.1, "capture")  # attacker being captured by defender
...
reach_set = np.maximum(goal1_destination, goal2_escape)
...
accuracy = "medium"
result = HJSolver(agents_1v1, grids, [reach_set, avoid_set], tau, compMethods, po, saveAllTimeSteps=True, accuracy=accuracy) # original one
...
# 6. Save the value function
np.save(f'MRAG/values/1vs1_easier_SIG_g{grid_size}_{accuracy}_dspeed1.5_Attacker.npy', result) 
